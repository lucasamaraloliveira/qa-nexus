...
import { Plus, Calendar, FileText, Pencil, Trash2, Save, Code, ChevronDown, ChevronRight, Eye, Download, Search, ChevronLeft, Copy, Check } from 'lucide-react';
...
export const VersionControl: React.FC<VersionControlProps> = ({ versions, setVersions }) => {
  ...
  const openAddScriptModal = (versionId: string) => {
  ...
  const handleAddScript = async () => {
    ...
    const scriptsToAdd: Script[] = [...pendingScripts];
  ...
  const handleSaveScript = async () => {
    ...
    const updatedScripts = version.scripts.map(s => {
      if (s.id === editingScript.script.id) {
        return { ...s, content: scriptContent };
      }
      return s;
    });
        ...
        <div className="flex flex-col h-[60vh]">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => editingScript && handleCopyScript(scriptContent, editingScript.script.id)}
              className="text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400"
            >
              {copiedScriptId === editingScript?.script.id ? (
                <Check className="w-4 h-4 mr-2" />
              ) : (
                <Copy className="w-4 h-4 mr-2" />
              )}
              {copiedScriptId === editingScript?.script.id ? 'Copiado!' : 'Copiar Conteúdo'}
            </Button>
          </div>
